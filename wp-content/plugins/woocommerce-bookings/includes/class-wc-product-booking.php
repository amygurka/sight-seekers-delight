...
class WC_Product_Booking extends WC_Product {
	...
	public function get_max_date() {
		$max_date['value'] = ! empty( $this->wc_booking_max_date ) ? apply_filters( 'woocommerce_bookings_max_date_value', absint( $this->wc_booking_max_date ) ) : 1;
	...
	public function get_available_blocks( $blocks, $intervals = array(), $resource_id = 0 ) {
		...
		if ( ! empty( $blocks ) ) {
			 ...
			 * @var array
			 */
			$existing_bookings = $this->get_bookings_in_date_range( $start_date, strtotime( "+{$base_interval} {$base_interval_unit}", $end_date ), $resource_id );
