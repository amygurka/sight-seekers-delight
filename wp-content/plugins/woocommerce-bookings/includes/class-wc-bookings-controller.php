...
class WC_Bookings_Controller {
	...
	public static function get_bookings_in_date_range( $start_date, $end_date, $product_or_resource_id = '' ) {
		...
		if ( false === ( $booking_ids = get_transient( $transient_name ) ) ) {
			$booking_ids = self::get_bookings_in_date_range_query( $start_date, $end_date, $product_or_resource_id );
	 ...
	 */
	private static function get_bookings_in_date_range_query( $start_date, $end_date, $product_or_resource_id = '' ) {
		global $wpdb;
		...
		$booking_ids = $wpdb->get_col( $wpdb->prepare( "
			...
			AND post_status IN ( 'unpaid', 'pending-confirmation', 'confirmed', 'paid', 'complete', 'in-cart' )
	...
	public static function get_latest_bookings( $numberitems = -1, $offset = 0 ) {
		$booking_ids = get_posts( array(
			...
			'order'       => 'DESC',
			'post_type'   => 'wc_booking',
			'post_status' => array( 'unpaid', 'pending-confirmation', 'confirmed', 'paid', 'cancelled' ),
			'fields'      => 'ids',
		) );
