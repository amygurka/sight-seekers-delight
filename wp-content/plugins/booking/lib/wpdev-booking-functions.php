...
//debuge($matches);            
    ...
    function wpbc_format_email_address( $wpbc_mail, $booking_form_show = array() ) {
        ...
        if  ( ( strpos($wpbc_mail, '<') === false ) &&
            ...
            $wpbc_mail = '"' . ((empty($wpbc_email_title))?__('Booking system','wpdev-booking'):substr($wpbc_email_title,0,-1)) . '" ' .   
                         '<' . $wpbc_mail . '>';
        }
        return $wpbc_mail;
    }
    ...
    function wpbc_wp_mail( $mail_recipient, $mail_subject, $mail_body, $mail_headers ){
        ...
        $mail_recipient = str_replace( '"', '', $mail_recipient );
...
//debuge( htmlspecialchars ($mail_body_to_send) );
    ...
    function sendNewBookingEmails($booking_id, $bktype, $formdata) {
        ...
        if ( strpos($mail_sender,'[visitoremail]') !== false ) {
            ...
            $wpbc_mail = '"' . ((empty($wpbc_email_title))?__('Booking system','wpdev-booking'):$wpbc_email_title) . '" '    
    ...
    function wpdev_ajax_check_bk_version(){
        ...
        if (!is_wp_error($result) && ($result['response']['code']=='200') && (true) ) {
           ...
           echo ' </script> ';
            ...
            echo '<h3>'; _e('Warning! Some error occur, during sending registration request.', 'wpdev-booking'); echo '</h3>';
            
            if (is_wp_error($result))  echo $result->get_error_message();
            else                       echo $result['response']['message'];
            echo '<br /><br />';
            _e('Please refresh this page and if the same error appear again contact support by email (with  info about order number and website) for finishing the registrations', 'wpdev-booking'); echo ' <a href="mailto:activate@wpbookingcalendar.com">activate@wpbookingcalendar.com</a>';
            echo '</strong></div>';
            echo ' <script type="text/javascript"> ';
