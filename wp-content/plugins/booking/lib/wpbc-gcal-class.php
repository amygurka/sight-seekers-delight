...
//        $url =  $scheme_and_host . $path . $query;               
        ...
        $this->show_message( __('Importing Feed','wpdev-booking') . ': ' . $url , true );
//debuge($url);        
...
//debuge($raw_data);
        ...
        if ( ! is_wp_error( $raw_data ) ) {
            ...
            if ( 200 == $raw_data['response']['code'] ) {
                    ...
                    $this->show_message( __('Data Parsing','wpdev-booking') , true );
...
//                                                    ."text^".trim($booking_gcal_events_form_fields['where'])."{$bktype}^{$location}"
                ...
                switch ( $raw_data['response']['code'] ) {
                    case 404:
                        $this->error = __( 'The feed could not be found (404). Please ensure your feed URL is correct.', 'wpdev-booking' );
                        break;
                    case 403:
                        $this->error = __( 'Access to this feed was denied (403). Please ensure you have public sharing enabled for your calendar.', 'wpdev-booking' );
                        break;
                    default:
                        $this->error = sprintf( __( 'The feed data could not be retrieved. Error code: %s. Please ensure your feed URL is correct.', 'wpdev-booking' ), $raw_data['response']['code'] );
                }
            }
        } else {
        ...
        if ( ! empty($this->error) ) {
            ...
            $this->show_message(  __('Done', 'wpdev-booking' ) );
    ...
    public function showImportedEvents( ) {
        ...
        <table style="width:100%;margin-top:45px;" id="gcal_imported_events<?php echo $this->getResource(); ?>" class="resource_table booking_table" cellpadding="0" cellspacing="0">
            ...
            <tr>
                ...
                <th style="height:35px;width:220px;" style="border-left: 1px solid #ccc;"> <?php _e('Title', 'wpdev-booking'); ?> </th>                
                <th style="text-align:center;"> <?php _e('Info', 'wpdev-booking'); ?> </th>
                <th style="text-align:center;"> <?php _e('Dates', 'wpdev-booking'); ?> </th>
                <th style="width:10px;height:35px;border-left: 1px solid #BBBBBB;"> <?php _e('GID', 'wpdev-booking'); ?> </th>
            </tr>
            <?php
            $alternative_color = '';
            if (! empty($this->events))
              foreach ($this->events as $bt) {
                ...
                ?>
                   <tr id="gcal_imported_events_id_<?php echo $bt['id']; ?>">
                        <td <?php echo $alternative_color; ?> ><span class="wpbc_mobile_legend"><?php _e('Selection', 'wpdev-booking'); ?>:</span><input type="checkbox" class="events_items" id="events_items_<?php echo $bt['id']; ?>" value="<?php echo $bt['id']; ?>"  name="events_items_<?php echo $bt['id']; ?>" /></td>
                        <td style="border-left: 1px solid #ccc;text-align: center;" <?php echo $alternative_color; ?> ><span class="wpbc_mobile_legend"><?php _e('ID', 'wpdev-booking'); ?>:</span><?php echo $bt['id']; ?></td>
                        <td <?php echo $alternative_color; ?> style="border-left: 1px solid #ccc;">
                            <span class="wpbc_mobile_legend"><?php _e('Title', 'wpdev-booking'); ?>:</span>
                        ...
                        <td style="border-right: 0px;border-left: 1px solid #ccc;text-align: center;" <?php echo $alternative_color; ?> >
                            <span class="wpbc_mobile_legend"><?php _e('Info', 'wpdev-booking'); ?>:</span>
                            <span ><?php echo $bt['description']; ?></span><br/>
                            <?php 
                            if (! empty($bt['location']) )
                                echo '<span >', __('Location:','wpdev-booking'), ': ' , $bt['location'], '</span>'; 
                            ?>
                        </td>

                        <td style="border-right: 0px;border-left: 1px solid #ccc;text-align: center;" <?php echo $alternative_color; ?> >
                            <span class="booking-listing-collumn booking-dates">
                                <span class="wpbc_mobile_legend"><?php _e('Dates', 'wpdev-booking'); ?>:</span>
                        ...
                        <td style="border-left: 1px solid #ccc;text-align: center;" <?php echo $alternative_color; ?> ><span class="wpbc_mobile_legend"><?php _e('GID', 'wpdev-booking'); ?>:</span><?php echo $bt['sync_gid']; ?></td>
                   </tr>                   
            <?php } ?>
            ...
            <tr class="wpbc_table_footer">
                <td colspan="6" style="text-align: center;"> 
                    <a href="javascript:void(0)" class="button button-primary" style="float:none;margin:10px;" 
                       onclick="javascript:location.reload();" ><?php _e('Reload page','wpdev-booking'); ?></a>                    
                    <a href="javascript:void(0)" class="button" style="float:none;margin:10px;" 
                       onclick="javascript:jQuery('#gcal_imported_events<?php echo $this->getResource(); ?>').remove();" ><?php _e('Hide','wpdev-booking'); ?></a>                    
                    <a href="javascript:void(0)" class="button"  style="float:none;margin:10px;"                                        
                       onclick="javascript: if ( bk_are_you_sure('<?php echo esc_js(__('Do you really want to delete selected booking(s) ?', 'wpdev-booking')); ?>') ) {
                                                    delete_booking( 
                       ...
                       ><?php _e('Delete selected booking(s)','wpdev-booking'); ?></i></a>
