...
//debuge('$dates_in_diff_formats["array]',$dates_in_diff_formats['array'])    ;
    ...
    $dates_results = $wpdb->get_results( $sql_req );
    ////////////////////////////////////////////////////////////////////////////
    
    if ( count($dates_results) == 0 )                                           // We do not have here booked dates at all,  so - TRUE
        return  true;
    
    $dates_array = array();
    foreach ($dates_results as $dates_obj) {
        
        if ( substr($dates_obj->booking_date, -1 ) == '0' ) {                   // This date fully booked, so not available - FALSE
                $is_days_free = false;
                break;                        
        }
        
        // we need to cut on 10 seconds for ability to sort times,  if its close to each  other.
        if ( substr($dates_obj->booking_date, -1 ) == '2' ) {
            $date = date('Y-m-d H:i:s', strtotime( '-10 second', strtotime( $dates_obj->booking_date ) ) );
        }
        if ( substr($dates_obj->booking_date, -1 ) == '1' ) {
            $date = date('Y-m-d H:i:s', strtotime( '+10 second', strtotime( $dates_obj->booking_date ) ) );
        }
...
//debuge('Booked Dates:',$dates_results);
//debuge('Transformed Dates:',$dates_array);
//debuge('Selected Dates:', $selected_dates_array);    

    if ( $is_days_free ) {

        sort($dates_array);

        $merged_array = array_merge($dates_array, $selected_dates_array);       // Merge selected and booked dates arrays
        
        sort($merged_array);

// debuge('Merged Dates Array: ', $merged_array);    


        // check all dates with  times must  be 1,2,1,2....,1,2
        $previous_time = intval( substr( $merged_array[0], -1 ) );
        
        for ( $i = 1; $i < count( $merged_array ); $i++ ) {
            $current_time = intval( substr( $merged_array[$i], -1 ) );
            if ( ( $current_time * $previous_time ) != 2 ) {
                $is_days_free = false;
// debuge('Error near date:', $merged_array[$i]);                
                break;                                        
            }
     ...
     ?> <script type="text/javascript">                 
            if ( jQuery('#submiting<?php echo $bktype; ?>' ).length ) {
              ...
              showMessageUnderElement( '#date_booking<?php echo $bktype; ?>',  
                                       '<?php echo html_entity_decode( '<strong>' . esc_js( __('Error!','wpdev-booking') ) . '</strong> ' 
                                               . esc_js( __('Probably these date(s) just was booking by other visitor. Please reload this page and make booking again.', 'wpdev-booking') ) ,ENT_QUOTES); ?>' , 
                                       'alert-error');
